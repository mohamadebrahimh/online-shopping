    ...
    public partial class ManageController : Controller
    {
        ...
        public virtual async Task<ActionResult> Index(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                message == ManageMessageId.ChangePasswordSuccess ? "کلمه عبور شما با موفقیت تغییر یافت."
                : message == ManageMessageId.SetPasswordSuccess ? "کلمه عبور شما با موفقیت تعیین گردید."
                : message == ManageMessageId.SetTwoFactorSuccess ? "احراز هویت دو مرحله ای برای شما اعمال گردید."
                : message == ManageMessageId.Error ? "خطایی رخ داده است."
                : message == ManageMessageId.AddPhoneSuccess ? "شماره تلفن شما اضافه گردید."
                : message == ManageMessageId.RemovePhoneSuccess ? "شماره تلفن شما حذف شد."
                : "";
        ...
        public virtual async Task<ActionResult> VerifyPhoneNumber(VerifyPhoneNumberViewModel model)
        {
            ...
            ModelState.AddModelError("", "تلفن شما تایید نشد");
            return View(model);
        }
        ...
        {
            ViewBag.StatusMessage =
                message == ManageMessageId.RemoveLoginSuccess ? "ورود از سیستم خارجی برداشته شد."
                : message == ManageMessageId.Error ? "خطایی رخ داده است."
                : "";
            var user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
            if (user == null)
        ...
        public virtual ActionResult LinkLogin(string provider)
        {
            ...
            return new Businessdevweb.Controllers.AccountController.ChallengeResult(provider, Url.Action("LinkLoginCallback", "Manage"), User.Identity.GetUserId());
        }
